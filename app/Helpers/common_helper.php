 ...
 * @return false|string
 */
function getInsertRegistDate()
{
    return date('Y-m-d H:i:s', time());
}

/**
 * UTC Date 를 지역 Date 로 변환
 *
 * @param $utc_date
 ...
 * @return mixed|string
 */
function getLocalDateToUTC($utc_date, $timezone = 'Asia/Seoul', $format = 'Y-m-d H:i:s')
{
    try {
 ...
 * @return mixed|string
 */
function getUTCTimeStampToLocalDate($local_date, $timezone = 'Asia/Seoul', $format = 'Y-m-d H:i:s')
{
    try {
        $dt = new DateTime($local_date, new DateTimeZone($timezone));
 ...
 * @return string
 */
function convertLocaleTimeWithTimezone(string $date, string $format = 'Y-m-d H:i:s') : string
{
    $request = \Config\Services::request();
    $param = $request->getJSON(true);
    ...
    try {
        ...
        $converted_date->setTimezone(new DateTimeZone($timezone));
    } catch (Exception $e) {
        return $date;
    }

    return $converted_date->format($format);
}

/**
 * @ymdhis : 2019-07-01 15:20:20
 * @minute : 추가시간 +, - 가능
 *
 * @param $ymdhis
 * @param $minute
 ...
 * @return false|string
 */
function BizDateMinuteAdd($ymdhis, $minute)
{
    $parsing = BizDataParsing($ymdhis);
    $ymd = $parsing[0];
    $his = $parsing[1];
    $datePar = explode("-", $ymd);
    $timePar = explode(":", $his);
    $seconds = mktime($timePar[0], $timePar[1] + $minute, $timePar[2], $datePar[1], $datePar[2], $datePar[0]);
 ...
 * @return array
 */
function BizDataParsing($ymdhis) : array
{
    $parsing = explode(" ", $ymdhis);
    $ymd = [];
    $ymd[0] = $parsing[0] ?? "1970-01-01";
    $ymd[1] = $parsing[1] ?? "00:00:00";
...
function parse_signed_request($signed_request) {
    ...
    $secret = "67d6ea3e9988bf33f6fe103d1442938d"; // Use your app secret here
...
function check24timeOver($time, $lang): array
{
    $response = [
        'status' => 'over',
        'limitTimeMsg' => ''
    ...
    $checkTime = $nowTime - $regTime;

    if ($checkTime < $limitTime) {
        $totalTime = $limitTime - $checkTime;
        ...
        $response['min'] = $min;
        $response['limitTimeMsg'] = lang('Common.call.reRequestTime', [$hours, $min], $lang);
    }
    return $response;
}
...
function checkBannedWord($str = '') : array
{
    $bannedWord = [];
    $response = [
        "status" => "error",
    ...
    foreach (BANNED_WORD as $key => $row) {
        $bannedWord = array_merge($bannedWord, $row);
    }
    
    foreach ($bannedWord as $len => $val) {
        if (preg_match("/".$val."/i", removeSpaceStr($str))) {
            $response['word'] = $val;
...
function checkStrLimitThreeNumber($str = '')
{
    if (preg_match("/(\d{1,}|.{1,})?\d{1,}(.{1,})?\d{1,}(.{1,})?\d{1,}/i", removeSpaceStr($str))) {
        return false;
    } else {
        return true;
    }
